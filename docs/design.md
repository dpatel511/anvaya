...
Schema version 3 retains the alternative and reference distance vectors for every unique edge-pair/substitution locus. It reports three parallel summaries: the original observation-weighted fraction, an equal-locus mean, and a locus-weighted mean capped at 20 observations per locus. Contributor and largest-edge fields expose concentration directly. These summaries are diagnostics for selecting and validating a later overdispersed decay model; the capped and equal-locus curves are not used as biological estimates or graph-cleaning inputs. Bins without observations are reported as `null` rather than zero.
