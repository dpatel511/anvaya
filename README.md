...
Evidence reporting is non-destructive. Weak tips and bounded incomplete branches are matched to an equal-length locally competing linear backbone when one is available. The report includes DNA and RY identity, relative coverage, substitutions, oriented terminal evidence, source-read linkage across substitutions, three heuristic scores, and an auditable classification. The optional JSON damage profile compares damage-compatible alternative and backbone observations by the exact changed-base cycle at unique matched weak-tip loci; incomplete branches remain excluded pending separate profile validation. Schema version 4 distinguishes all structurally matched loci from loci with changed-base observations inside the requested window, retains per-locus counts, and reports raw observation-weighted, equal-locus, and coverage-capped summaries. These parallel estimates diagnose coverage sensitivity; none is yet a calibrated whole-library model. Linkage strengthens only multi-substitution evidence; neither linkage nor the empirical profile alone establishes biochemical causality. Reporting does not remove or retain paths automatically and does not require a supplied damage profile.
